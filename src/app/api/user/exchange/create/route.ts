...
export async function POST(req:NextRequest){
    ...
    const {account_number,from_amount_code,from_current_amount,from_current_img,from_current_name,to_amount_code,
       to_current_amount,to_current_img,to_current_name,user_account_number,payment_proof,userId,number_from_payment}:IExchange = await req.json()
...
try {
    ...
    const dataExhangeForm:IExchange = {
       ...
       status:"waiting",
       user_name:String(userOrder?.firstname),
       user_username:String(userOrder?.username),
       user_avator:String(userOrder?.avator),
       number_from_payment
    }
  ...
  if(!makeExchange){
    ...
    return NextResponse.json({success:false,error:`there is error with create exchange: ${(error as Error).name} Please contact support`},{status:400})
}
