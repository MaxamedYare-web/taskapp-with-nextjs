...
export async function PUT(req:NextRequest){
    ...
    try {
        ...
        if(!formDataFile){
            return NextResponse.json({error:"file not found please try to choose file"},{status:401})
        }
...
return NextResponse.json({success:true,message:"Successfully the profile was updated",display_url:dataImage.data.display_url},{status:200})
        ...
        return NextResponse.json({error:`there is error with uploading image: ${(error as Error).message}`},{status:500})
    }
}
