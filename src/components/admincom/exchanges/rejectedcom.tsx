...
export default function RejectedCom() {
    const [isPending, startTransition] = useTransition()
    const [refundExchange, setrefundExchange] = useState<IExchange[]>([])
    ...
    useEffect(() => {
        startTransition(async () => {
            ...
            if (result.listExchnage) {
                ...
                const filterPen = pendingArr.filter((p) => p.status == "rejected")

                setrefundExchange(filterPen)
            }
        })
    }, [startTransition])
    ...
    type exchangeApprove = (typeof refundExchange)[0]
    ...
    const exchangeListner = useCallback((pendingLists: exchangeApprove, columKey: React.Key) => {
        ...
        switch (columKey) {
            ...
            case "status":
                return <h1 className="text-danger-500 capitalize flex items-center bg-danger-100 p-1 rounded gap-1 w-25 justify-center font-bold"><Badge color="red" /> {pendingLists.status}</h1>
            case "action":
                return <Button as={Link} href={`/admin/rejected/${pendingLists.id}`} variant="ghost"><Eye />Veiw</Button>
        }
    ...
    const pagePerRows = 7
    const pages = Math.ceil(refundExchange?.length / pagePerRows)

    const refundItems = useMemo(() => {
        const start = (currentPage - 1) * pagePerRows
        const end = start + pagePerRows
        return refundExchange?.slice(start, end)
    }, [refundExchange, currentPage])
            ...
            <main className="p-3 space-y-3">
                <div className="flex justify-between p-2 rounded bg-default-50">
                    <article className="w-full">
                        ...
                        <p className="text-default-500">Refund Exchange lists</p>
                    </article>
                    <Input placeholder="Searching exchange" />
                </div>

                {/* table */}
                <Table aria-label="refund lists"
                    bottomContent={
                ...
                >
                    ...
                    {
                        refundItems.length == 0 ? <TableBody isLoading={isPending} emptyContent={"there is no found pending"}>
                            {[]}
                        </TableBody> :
                            <TableBody isLoading={isPending} items={refundItems}>
                                {
                                    (exchangeItem) => (
