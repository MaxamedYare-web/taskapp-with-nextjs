...
export default function RefundCom() {
    ...
    useEffect(() => {
        startTransition(async () => {
            ...
            if (result.listExchnage) {
                ...
                const filterPen = pendingArr.filter((p) => p.status == "refund")
    ...
    const exchangeListner = useCallback((pendingLists: exchangeApprove, columKey: React.Key) => {
        ...
        switch (columKey) {
            ...
            case "status":
                return <h1 className="text-warning-500 capitalize flex items-center bg-warning-100 p-1 rounded gap-1 w-25 justify-center font-bold"><Badge color="yellow" /> {pendingLists.status}</h1>
            case "action":
                return <Button as={Link} href={`/admin/refund/${pendingLists.id}`} variant="ghost"><Eye />Veiw</Button>
        }
